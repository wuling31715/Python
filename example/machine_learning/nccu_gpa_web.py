...
def get_data(id, password):

    browser.get('https://i.nccu.edu.tw/Login.aspx?ReturnUrl=%2fdefault.aspx')
    browser.maximize_window()

    browser.find_element_by_id('captcha_Login1_UserName').clear()
    browser.find_element_by_id('captcha_Login1_UserName').send_keys(id)
    ...
    time.sleep(1)
    browser.get(
        'http://i.nccu.edu.tw/sso_app/PersonalInfoSSO.aspx?p=2&sid=%s' % (id))
    time.sleep(3)
...
#    executable_path='/Users/dannyshau/code/python/driver/chromedriver')

browser.maximize_window()
data_all_str = get_data('103306071', 'nccu1015')
